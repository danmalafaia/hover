...
FROM ubuntu:jammy AS flutterbuilder
RUN apt-get update \
    && apt-get install -y \
        git curl unzip
...
FROM ubuntu:jammy AS xarbuilder
RUN apt-get update \
	&& apt-get install -y \
...
FROM ubuntu:jammy AS bomutilsbuilder
RUN apt-get update \
	&& apt-get install -y \
	    git make g++
...
FROM ubuntu:jammy as appimagebuilder
RUN apt-get update \
	&& apt-get install -y \
	    curl
RUN cd /opt \
...
FROM ubuntu:jammy AS pacmanbuilder
